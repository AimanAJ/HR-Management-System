...
const modal = new bootstrap.Modal(document.getElementById("new-task-form"));
formFields.startDate.value = new Date().toISOString().split("T")[0];
...
function selectEmployeeToTheTask(employeeName) {
  ...
  // Create a badge to display the selected employee using template literals
  const badgeHtml = `
    <span class="badge text-bg-secondary pointer me-1" id="${employeeName}">
      ${employeeName} <i class="bi bi-x"></i>
    </span>
  `;
  selectedEmployeesSpan.insertAdjacentHTML('beforeend', badgeHtml);

  selectedEmployeesSpan.lastElementChild.addEventListener("click", (event) => {
    selectedEmployees = selectedEmployees.filter((val) => val !== employeeName);
    event.currentTarget.remove();
    selectEmployee.required = selectedEmployees.length === 0;

    // Unhide the employee option
    document.querySelector(`option[value="${employeeName}"]`).style.display = "";
  });

  selectEmployee.required = false;
  selectEmployee.value = "";
  document.querySelector(`option[value="${employeeName}"]`).style.display = "none";
}
...
async function getData() {
  ...
  const jsonData = await response.json();
  jsonData.forEach((employee) => {
    selectEmployee.insertAdjacentHTML('beforeend', `<option value="${employee["Full Name"]}">${employee["Full Name"]}</option>`);
  });
}
...
function deleteTaskCard(task) {
  document.querySelector(`#task-card-${task.id}`).remove();
}
...
function addTaskToPage(task) {
  const tasksGroup = document.querySelector(`.tasks-group.${task.status}`);
  tasksGroup.querySelector(".tasks-container").insertAdjacentHTML('beforeend', createTaskCard(task));

  const taskGroupHeader = tasksGroup.querySelector(".tasks-group-header span");
  const taskCounter = parseInt(taskGroupHeader.textContent.replace(/[^\d]/g, "")) + 1;
  taskGroupHeader.textContent = `(${taskCounter})`;
}
...
function fillFormWithTaskData(task) {
  ...
  formFields.description.value = task.description;
  document.querySelector(`input[value="${task.priority}"]`).checked = true;
  task.assignMembersSelect.forEach(selectEmployeeToTheTask);
}

tasks.forEach(addTaskToPage);

modal.addEventListener("hidden.bs.modal", restForm);
modal.addEventListener("show.bs.modal", () => {
  document.getElementById("new-task-formLabel").innerText = editMode ? "Edit Task" : "Add Task";
});
